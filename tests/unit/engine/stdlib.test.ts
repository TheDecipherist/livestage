...
// Confirms stdlib macros are available in every .stage document without any @import.
...
describe('stdlib — auto-loaded macros', () => {
  ...
  it('stdlib macros are available without @import', () => {
    ...
    const result = run(`@call git-branch /
...
`)
  ...
  it('git-status macro runs without errors', () => {
    const result = run(`@call git-status /
`, true)
    expect(result.errors).toHaveLength(0)
  })

  it('git-log macro produces commit history', () => {
    const result = run(`@call git-log /
`, true)
    expect(result.errors).toHaveLength(0)
  ...
  it('project-manager detects a package manager', () => {
    const result = run(`@call project-manager /
manager: {{ pkg_manager }}
`, true)
    expect(result.errors).toHaveLength(0)
  ...
  it('env-node reports a version string', () => {
    const result = run(`@call env-node /
node: {{ node_version }}
`, true)
    expect(result.errors).toHaveLength(0)
...
`, true)
  ...
  it('fs-count with ext parameter works', () => {
    const result = run(`@call fs-count ext=ts /
count: {{ file_count }}
`, true)
    expect(result.errors).toHaveLength(0)
  ...
  it('code-grep with pattern parameter works', () => {
    const result = run(`@call code-grep pattern=execute /
...
`, true)
  ...
  it('user-defined macro overrides stdlib macro', () => {
    ...
    const result = run(`@define project-manager
...
`, true)
  ...
  it('env-has with cmd parameter returns true or false', () => {
    const result = run(`@call env-has cmd=node /
...
`, true)
  ...
  it('code-any-types returns a number', () => {
    const result = run(`@call code-any-types /
any count: {{ any_count }}
`, true)
    expect(result.errors).toHaveLength(0)
