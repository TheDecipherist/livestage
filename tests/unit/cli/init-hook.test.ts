...
import { isLiveStageDocument, REDIRECT_MESSAGE, HOOK_SCRIPT } from '../../../src/cli/commands/init.js'

describe('isLiveStageDocument', () => {
  it('detects bare @markdownai header on first line', () => {
    expect(isLiveStageDocument('@markdownai v1.0\nbody\n')).toBe(true)
  })

  it('detects @markdownai after leading blank lines', () => {
    expect(isLiveStageDocument('\n\n@markdownai v1.0\nbody\n')).toBe(true)
  })
  ...
  it('detects @markdownai after YAML frontmatter (Claude Code slash-command shape)', () => {
    ...
    expect(isLiveStageDocument(content)).toBe(true)
  })
  ...
  it('rejects a plain Markdown file with no @markdownai', () => {
    expect(isLiveStageDocument('# Just a regular doc\n\nNothing special.\n')).toBe(false)
  })
  ...
  it('rejects a file with frontmatter but no directive lines anywhere', () => {
    ...
    expect(isLiveStageDocument(content)).toBe(false)
  })
  ...
  it('detects any markdownai directive (@phase, @if, @call, @plugin-meta, etc.)', () => {
    expect(isLiveStageDocument('# Doc\n@phase setup\n')).toBe(true)
    expect(isLiveStageDocument('# Doc\n@if {{ x }}\n')).toBe(true)
    expect(isLiveStageDocument('# Doc\n@call my-macro\n')).toBe(true)
    expect(isLiveStageDocument('# Doc\n@plugin-meta\n')).toBe(true)
    expect(isLiveStageDocument('# Doc\n@markdownai-detect\n')).toBe(true)
  ...
  it('rejects an empty file', () => {
    expect(isLiveStageDocument('')).toBe(false)
    expect(isLiveStageDocument('\n\n\n')).toBe(false)
  })
})
...
describe('REDIRECT_MESSAGE', () => {
  ...
  it('opens with ironclad imperative framing', () => {
    ...
    expect(REDIRECT_MESSAGE).toContain('I MUST forward it to the LiveStage MCP server')
  })
...
describe('PreToolUse hook (end-to-end via spawn)', () => {
  ...
  it('allows non-Read tools (e.g. Edit) to pass through even on LiveStage files', () => {
    const docPath = join(projectDir, 'doc.md')
    writeFileSync(docPath, '@markdownai v1.0\n\n# Hello\n', 'utf8')
    const out = runHookWith({
      tool_name: 'Edit',
