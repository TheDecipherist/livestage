 ...
 *   - When <cwd>/CLAUDE-LiveStage.md exists, the hook spawns `mai render`
...
describe('SessionStart hook (renders CLAUDE-LiveStage.md, injects additionalContext)', () => {
  ...
  it('exits 0 silently when CLAUDE-LiveStage.md does not exist', () => {
    const out = runHook({ cwd: projectDir })
    expect(out.code).toBe(0)
    expect(out.stdout).toBe('')
    expect(out.stderr).toBe('')
    expect(existsSync(join(projectDir, 'CLAUDE.md'))).toBe(false)
    expect(existsSync(join(projectDir, 'CLAUDE-LiveStage.md'))).toBe(false)
  })

  it('does NOT touch CLAUDE.md when CLAUDE-LiveStage.md is present', () => {
    const claudeMd = '# My project rules\n\nDo not touch this file.\n'
    writeFileSync(join(projectDir, 'CLAUDE.md'), claudeMd, 'utf8')
    writeFileSync(join(projectDir, 'CLAUDE-LiveStage.md'), '@markdownai v1.0\nhello\n', 'utf8')
  ...
  it('emits hookSpecificOutput JSON with the rendered content on stdout', () => {
    ...
    expect(parsed.hookSpecificOutput.additionalContext).toMatch(/^RENDERED:.+CLAUDE-LiveStage\.md/)
  })
  ...
  it('falls back to process.cwd() when stdin cwd is missing', () => {
    // No CLAUDE-LiveStage.md in process.cwd() (we're running from the workspace
