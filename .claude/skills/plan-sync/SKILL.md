...
Mirror the steps to the status bar: run `node .claude/hooks/lib/statusbar.cjs set plan-sync <step> <total> "<label>"` alongside each line and `node .claude/hooks/lib/statusbar.cjs done plan-sync` with the DONE line (pre-approved, best-effort, silent).
