...
known_issues:
  ...
  - "OUT OF SCOPE, reported not fixed: `checkShellCommand`'s prefix-wildcard pattern matching (src/engine/security/rules.ts's `matchShellPattern`, turning a trailing `*` into an unanchored `.*`) is a genuine, PRE-EXISTING command-injection-capable weakness affecting more than this feature: `defaultSecurityConfig()` (src/engine/security/config.ts, used whenever no project `.livestage/policy.json` exists at all) itself ships `git *`, `cat *`, `find *`, `grep *`, and several other prefix-wildcard patterns as its curated default allowlist, so any project relying on the framework's own default (not just this feature's example) is exposed to the same class of injection the moment a @query command in that project ever interpolates `{{ }}`/`${}` into a `git *`-shaped command. Fixing this is a security-critical change to feature 06 (CR-5, Deny By Default) / feature 10 (Security Policy Core)'s core contract, touching the shared pattern-matching primitive every policy in the project relies on; it deserves its own dedicated, carefully-reviewed security-hardening build, not a side-fix bundled into a docs-generation feature. Flagged for the user to decide how to prioritize."
---
