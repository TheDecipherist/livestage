...
import { checkAbsolutePath } from 'livestage/engine'
...
/**
 * Detect whether a file is a LiveStage document.
 *
 * Two valid forms:
 ...
 */
export function isLiveStageDocument(content: string): boolean {
  ...
  // with a letter/underscore) marks the file as containing LiveStage
  // directive syntax. Claude must never see these raw — the engine runs
  // them and substitutes the result.
  //
  // The pattern matches LiveStage directives (@phase, @if, @call,
 ...
 * LiveStage document. Designed to be self-contained - lists every MCP tool
...
export const REDIRECT_MESSAGE = [
  ...
  'I MUST forward it to the LiveStage MCP server.',
  '════════════════════════════════════════════════════════════════════',
  '',
  'This file is a LiveStage document (it contains an `@markdownai` header).',
  ...
  'NOT the content. The content only exists AFTER the LiveStage engine has',
  ...
  'any tool other than the LiveStage MCP server below. There is no fallback,',
  'no manual interpretation, no inline-execute — the MCP server is the ONLY',
  'way to get the content of a LiveStage document.',
  '',
  ...
  '',
  'NOTE on CLAUDE-LiveStage.md specifically:',
  '  If the blocked file is named `CLAUDE-LiveStage.md`, its rendered content',
  ...
  '  use `mcp__markdownai__read_file` with `{ path: "CLAUDE-LiveStage.md",',
  ...
  '            call this first when opening a new LiveStage document.',
  '',
  ...
  '            This is the main way you READ a LiveStage document.',
  '',
  ...
  '              format?: \'ai\'|\'standard\' }',
  '   returns: { content: string, isLiveStage: boolean, warnings: string[] }',
  '   use:     friendly alias for read_file with `file` parameter (consistent',
  '            with list_phases / resolve_phase / call_macro / get_constraints).',
  '            Renders a full LiveStage document with all directives executed.',
  '',
  ...
  '   args:    { filePath: string, cwd: string }',
  '   returns: { constraints: Array<{id, severity, body}>, isLiveStage: boolean, blocked?: boolean }',
  ...
  'They are the entire surface area for working with LiveStage documents.',
  ...
  'is the right answer.',
  '',
  'TYPICAL WORKFLOW for opening a LiveStage document:',
  '',
...
function isLiveStageDocument(content) {
...
try {
  ...
  if (!isLiveStageDocument(content)) process.exit(0)
...
function ensureHookFile(hookDir: string, hookPath: string): void {
  ...
  const hookAlreadyExists = existsSync(hookPath) &&
    readFileSync(hookPath, 'utf8').includes('LiveStage PreToolUse hook')
 ...
 *     this session.
 *   - `CLAUDE-LiveStage.md` is a separate file the user creates and edits.
 *     It contains `@markdownai` directives that resolve at session start.
 *   - All directives in `CLAUDE-LiveStage.md` are rendered in one shot via
 ...
 *   - Direct Read of `CLAUDE-LiveStage.md` is intercepted by the PreToolUse
 *     hook (it's a LiveStage doc). Claude must use the MCP server if it
 *     ever needs to re-fetch the rendered content mid-session.
 *
 * No-op when:
 *   - `CLAUDE-LiveStage.md` doesn't exist (project hasn't adopted the feature)
...
// LiveStage SessionStart hook - installed by mai init
//
// Renders <cwd>/CLAUDE-LiveStage.md via the mai CLI and injects the result
// into Claude's session context via hookSpecificOutput.additionalContext.
//
// CLAUDE.md is never touched. CLAUDE-LiveStage.md is a separate file the
...
//
// No-op when CLAUDE-LiveStage.md doesn't exist or mai render fails.
// Never blocks session start (always exits 0).
import { createInterface } from 'node:readline'
...
try {
  ...
  const filePath = join(cwd, 'CLAUDE-LiveStage.md')
  ...
  if (result.error) {
    ...
    process.stderr.write('LiveStage SessionStart hook: cannot invoke "mai render" (' + String(result.error) + '). Session continues without CLAUDE-LiveStage.md context.\\n')
    process.exit(0)
  }

  if (result.status !== 0) {
    process.stderr.write('LiveStage SessionStart hook: mai render failed for CLAUDE-LiveStage.md. Session continues without injected context.\\n')
...
function updateClientHooks(configPath: string, hookPath: string, sessionStartHookPath: string): HookUpdateResult {
  ...
  // PreToolUse: block direct Read of LiveStage documents and redirect
  ...
  // SessionStart: render CLAUDE-LiveStage.md and inject as additionalContext
