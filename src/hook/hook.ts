...
function startsWithLiveStage(content: string): boolean {
  ...
  try {
    ...
    try {
      ...
      if (peek.trimStart().startsWith(MAI_HEADER)) {
        return { route: 'mcp', reason: 'LiveStage document detected' }
      }
      ...
      if (peek.trimStart().startsWith('---')) {
        ...
        if (startsWithLiveStage(content)) {
          return { route: 'mcp', reason: 'LiveStage document detected (after YAML frontmatter)' }
        }
      }

      return { route: 'passthrough', reason: 'Not a LiveStage document' }
    } finally {
      closeSync(fd)
    }
  } catch (err) {
...
export function isLiveStageFile(filePath: string): boolean {
  if (!filePath.endsWith('.md')) return false
  if (!isSafeHookPath(filePath)) return false
  try {
    const content = readFileSync(filePath, 'utf8')
    return startsWithLiveStage(content)
  } catch (err) {
    process.stderr.write(`[markdownai] hook: cannot read file "${filePath}": ${String(err)}\n`)
    return false
  }
