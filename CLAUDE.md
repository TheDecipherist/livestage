...
- Session analysis, benchmark reports, and other one-off writeups are
  always saved as a markdown file under `./.ai_temp/`, never published as
  a claude.ai artifact. Established after a benchmark report got
  published as an artifact and also committed at the repo root by
  mistake; both were wrong, the file belongs in the gitignored scratch
  directory alongside the rest of a session's working output, not in git
  history and not on a hosted page nobody asked for.
